...
class Preprocessing(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        aud, _ = librosa.load(os.path.join(file_path.replace(args.Landmark, args.Data_dir).replace('GRID_imgs_aud', 'GRID_imgs_aud_again').replace('video', 'audio')[:-4] + '.wav'), 16000)
...
for kk, data in enumerate(Data_loader):
    ...
    save_path_temp = save_path + f'_temp'
    f_name = f_name[0]
    if not os.path.exists(save_path):
        os.makedirs(save_path)
    if not os.path.exists(save_path_temp):
        os.makedirs(save_path_temp)
    torchvision.io.write_video(os.path.join(save_path_temp, f_name[:-4] + '.mp4'), video_array=cropped_video, fps=args.FPS)
    sf.write(os.path.join(save_path_temp, f_name[:-4] + '.flac'), aud.numpy(), samplerate=16000)
    vid = os.path.join(save_path_temp, f_name[:-4] + '.mp4')
    aud = os.path.join(save_path_temp, f_name[:-4] + '.flac')
    commend = f"ffmpeg -i {vid} -i {aud} -c:a aac -c:v libx264 -r {args.FPS} -map 0:v:0 -map 1:a:0 -strict -2 {os.path.join(save_path, f_name[:-4] + '.mp4')}"
    subprocess.call(commend, shell=True)
    os.remove(vid)
    os.remove(aud)
    print('##########', kk + 1, ' / ', len(Data_loader), '##########')

temp_dirs = sorted(glob.glob(os.path.join(args.Output_dir, '*', '*_temp')))
